...
public class MapMerger {
      ...
      public static <K, V> Map<K, Map.Entry<V, V>> mergeMaps(List<Map<K, V>> list1, List<Map<K, V>> list2) {
            ...
            // Первая карта для быстрого поиска ключей из первого списка карт
            Map<K, V> firstMap = new HashMap<>();
            for (Map<K, V> map : list1) {
                  firstMap.putAll(map);
            }

            // Вторая карта для быстрого поиска ключей из второго списка карт
            for (Map<K, V> map : list2) {
                  for (Map.Entry<K, V> entry : map.entrySet()) {
                        if (firstMap.containsKey(entry.getKey())) {
                              result.put(entry.getKey(), new AbstractMap.SimpleEntry<>(firstMap.get(entry.getKey()), entry.getValue()));
                        }
                  }
            }
